...
/**
 * Reduced Version of the Ts3SyncAPI.
 * Just every Method with its basic Data Types.
 * Not much Methodoverriding, just the basic Method
...
public class Ts3BasicAPI {
	...
	public void connectTeamSpeakQuery(int serverID, String nickName) {
		if (!isConnected()) {
			selectVirtualServer(serverID, nickName); // select the virtualServer, the query client should connect to
	...
	/**
	 * Uses only the given information of clientdblist. Less detailed than databaseclientinfo
	 * for each client, because some Information are not provided by clientdblist.
	 * 
	 * @see Ts3SyncAPI#getClientDBListDetailed() gets more information about each client.
	 * 
	 ...
	 * 
	 * @see Ts3SyncAPI#getOnlineClientsDetailed() gets more information about each client.
	 * 
	...
	public List<ChannelInfo> getChannelsByName(String channelName) {
		...
		for (String channel : channels) {
			ChannelInfo chInfo = new ChannelInfo(channel.split(" "));
			chInfo.addInfo("cid", get(channel, "cid="));
	...
	/**
	 * Provides a Map which contains every Clientdatabaseid
	 * as a ley and the value is a list of channelids
	 * in which the Clientdatabaseid has the channelgroup.
	 * 
	...
	public void stopVirtualServer(int virtualServerID, String reasonmsg) {
		query.getWriter().executeReadErrorCommand("serverstop sid=" + virtualServerID + " reasonmsg=" + Formatter.toTsFormat(reasonmsg));
	}
	
	
	public void stopVirtualServer(int virtualServerID) {
		query.getWriter().executeReadErrorCommand("serverstop sid=" + virtualServerID);
	}
	...
	public void addChannelClientPermission(int channelID, int clientdataBaseID, int permissionID, String permissionName, int permissionValue) {
		query.getWriter().executeReadErrorCommand("channelclientaddperm cid=" + channelID + " cldbid=" + clientdataBaseID + " permid=" + permissionID + " permsid="
				+ Formatter.toTsFormat(permissionName) + " permvalue=" + permissionValue);
	}
	
	public void addChannelClientPermission(int channelID, int clientdataBaseID, int permissionID, int permissionValue) {
		query.getWriter().executeReadErrorCommand("channelclientaddperm cid=" + channelID + " cldbid=" + clientdataBaseID + " permid=" + permissionID + " permvalue=" + permissionValue);
	}
	...
	public void addChannelGroupPermission(int channelGroupID, int permissionID, String permissionName, int permissionValue) {
		query.getWriter().executeReadErrorCommand(
				"channelgroupaddperm cgid=" + channelGroupID + " permid=" + permissionID + " permsid=" + Formatter.toTsFormat(permissionName) + " permvalue=" + permissionValue);
	}

	public void addChannelGroupPermission(int channelGroupID, int permissionID, int permissionValue) {
		query.getWriter().executeReadErrorCommand("channelgroupaddperm cgid=" + channelGroupID + " permid=" + permissionID + " permvalue=" + permissionValue);
	}
	...
	public void removeChannelGroupPermission(int channelGroupID, int permissionID, String permissionName) {
		query.getWriter().executeReadErrorCommand("channelgroupdelperm cgid=" + channelGroupID + " permid=" + permissionID + " permsid=" + Formatter.toTsFormat(permissionName));
	}

	public void removeChannelGroupPermission(int channelGroupID, int permissionID) {
		query.getWriter().executeReadErrorCommand("channelgroupdelperm cgid=" + channelGroupID + " permid=" + permissionID);
	}
	...
	public void addClientPermission(int clientDataBaseID, int permissionID, String permissionName, int permissionValue, boolean permSkip) {
		query.getWriter().executeReadErrorCommand("clientaddperm cldbid=" + clientDataBaseID + " permid=" + permissionID + " permsid=" + Formatter.toTsFormat(permissionName) + " permvalue="
				+ permissionValue + " permskip=" + Formatter.toInt(permSkip));
	}

	public void addClientPermission(int clientDataBaseID, int permissionID, int permissionValue, boolean permSkip) {
		query.getWriter()
				.executeReadErrorCommand("clientaddperm cldbid=" + clientDataBaseID + " permid=" + permissionID + " permvalue=" + permissionValue + " permskip=" + Formatter.toInt(permSkip));
	}
	...
	public void kickClientFromServer(int clientID, String reason) {
		query.getWriter().executeReadErrorCommand("clientkick reasonid=5 reasonmsg=" + Formatter.toTsFormat(reason) + " clid=" + clientID);
	}

	public void kickClientFromServer(int clientID) {
		query.getWriter().executeReadErrorCommand("clientkick reasonid=5 clid=" + clientID);
	}
	
	public void kickClientFromChannel(int clientID, String reason) {
		query.getWriter().executeReadErrorCommand("clientkick reasonid=4 reasonmsg=" + Formatter.toTsFormat(reason) + " clid=" + clientID);
	}

	public void kickClientFromChannel(int clientID) {
		query.getWriter().executeReadErrorCommand("clientkick reasonid=4 clid=" + clientID);
	}
	...
	public List<PermissionAssignmentInfo> getPermOverview(int clientDBID, int channelID) {
		...
		String[] info = query.getWriter().executeReadCommand("permoverview cldbid=" + clientDBID + " cid=" + channelID + " permid=0")[0].split(TS_INFO_SEPERATOR);
		for (String perms : info) {
			resultList.add(new PermissionAssignmentInfo(perms.split(" ")));
		}
	...
	public PermissionAssignmentInfo getPermOverviewByPermID(int clientDBID, int channelID, int permID) {
		...
		String[] info = query.getWriter().executeReadCommand("permoverview cldbid=" + clientDBID + " cid=" + channelID + " permid=" + permID)[0].split(TS_INFO_SEPERATOR);
		for (String perms : info) {
			resultList.add(new PermissionAssignmentInfo(perms.split(" ")));
		}
