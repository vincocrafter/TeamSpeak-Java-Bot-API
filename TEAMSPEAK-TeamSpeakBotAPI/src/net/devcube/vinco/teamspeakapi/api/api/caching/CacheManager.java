...
public class CacheManager {
    ...
    protected void addChannelListCacheChannel(String information) {
        cache.put(CHANNEL_LIST, getChannelsList().concat("|").concat(information));
    }

    protected String getChannelListChannel(int channelID) {
        for (String channels : getChannelsList().split(TS_INFO_SEPARATOR)) {
    ...
    protected void updateChannelListChannelProp(int channelID, String information) {
    ...
    protected void updateChannelCache(int channelID, String information) {
        cache.put(CHANNEL + SEPARATOR + channelID, information); // Update individual channel
    }
    ...
    protected void updateDBClientCache(int clientDBID, String information) {
    ...
    public String updateAttribute(String information, String splitter, String newValue) {
        if (!information.contains(splitter))
            return information;

        String[] split = information.split(splitter);
        String end = split[1].substring(split[1].split(" ")[0].length());
        StringBuilder res = new StringBuilder(split[0]);
        if (newValue.isEmpty()) {
            res.append(splitter);
        } else {
            res.append(splitter).append("=").append(Formatter.toTsFormat(newValue));
        }
